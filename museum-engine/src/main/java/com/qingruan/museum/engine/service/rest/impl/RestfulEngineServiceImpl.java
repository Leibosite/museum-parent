...
import com.qingruan.museum.engine.service.rest.domain.EnvironmentBean;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentHistoryBean;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentHistoryMessage;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentListBean;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentListMessage;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentMessage;
import com.qingruan.museum.engine.service.rest.domain.EnvironmentSettingBean;
import com.qingruan.museum.engine.service.rest.domain.PolicyGroupListMsg;
import com.qingruan.museum.engine.service.rest.domain.PolicyGroupMsg;
import com.qingruan.museum.engine.service.rest.domain.PolicyGroupResponse;
import com.qingruan.museum.engine.service.rest.domain.PolicyListMsg;
import com.qingruan.museum.engine.service.rest.domain.PolicyMsg;
import com.qingruan.museum.engine.service.rest.domain.PolicyResponse;
import com.qingruan.museum.engine.service.rest.domain.ResponseBaseMsg;
import com.qingruan.museum.engine.service.rest.fetch.Message;
import com.qingruan.museum.engine.service.rest.fetch.MessageBase;
...
public class RestfulEngineServiceImpl implements RestfulEngineService {
	...
	public ByteBuf fetchConstantTh(String constantThName, String token) {
		...
		try {
			...
			if (StringUtils.isBlank(constantThName)) {
				...
				throw new WebApiException(
						WebApiExceptionType.MISS_CONSTANT_TH_NAME);
			}
			...
			List<Device> fetchConstantThByName = this
					.fetchConstantThByName(constantThName);
	...
	public ByteBuf fetchShowCase(String showCaseName, String token) {
		...
		try {
			...
			if (StringUtils.isBlank(showCaseName)) {
				...
				throw new WebApiException(
						WebApiExceptionType.MISS_SHOW_CASE_NAME);
			}
			...
			List<RepoArea> fetchShowCaseByName = this
					.fetchShowCaseByName(showCaseName);
	...
	public ByteBuf fetchEmailTemplate(String emailTemplateName, String token) {
		...
		try {
			...
			if (StringUtils.isBlank(emailTemplateName)) {
				...
				throw new WebApiException(
						WebApiExceptionType.MISS_EAMIL_TEMPLATE_NAME);
			}
			...
			List<EmailTemplate> fetchEmailTemplateByName = this
					.fetchEmailTemplateByName(emailTemplateName);
	...
	@Transactional(readOnly = false)
	private List<EmailTemplate> fetchEmailTemplateByName(
	...
	private List<Device> fetchConstantThByName(String constantThName) {
		...
		List<Device> findConstantThName = deviceDao
				.findConstantTh(constantThName);
	...
	private List<RepoArea> fetchShowCaseByName(String showCaseName) {
		...
		List<RepoArea> findShowCaseNameName = repoAreaDao
				.findShowCase(showCaseName);
	...
	@Override
	public ByteBuf processOperatingMode(String idString, String token,
			String operator) {
		log.info("开始处理运行状态设定");
		...
		try {
			...
			if (operator == null
					|| !(operator.equals("debug") || operator
							.equals("production"))) {
			...
			if (operator.equals("production")) {
				sensorOperatingStatus
						.setOperatingMode(OperatingMode.PRODUCTION);
			}
	...
	@Override
	public ByteBuf historyRecordReport(String idString, String token,
			String operator) {
		...
		try {
			...
			if (StringUtils.isBlank(idString)) {
				...
				if (device == null
						|| !(device.getDeviceType().equals(
								DeviceType.MONITORING_POINT_ONE) || device
								.getDeviceType().equals(
										DeviceType.MONITORING_POINT_TWO))) {
					log.info("设定参数错误 id");
					throw new WebApiException(WebApiExceptionType.ID_ERROR);
				}
			}
			if (operator == null
					|| !(operator.equals("start") || operator.equals("stop"))) {
				log.info("设定参数错误 operator");
				throw new WebApiException(WebApiExceptionType.OPERATOR_ERROR);
			}
			...
			sensorHistoryRecord.setTopologyPath(getTopologyPath(device
					.getParentId()));

			if (operator.equals("start")) {
				sensorHistoryRecord
						.setHistoryReportMode(HistoryReportMode.START);
			}
			if (operator.equals("stop")) {
				sensorHistoryRecord
						.setHistoryReportMode(HistoryReportMode.STOP);
			}
	...
	@Override
	public ByteBuf setConstantTh(String id, String presentValue,
			String topLimit, String lowerLimit, String fluctuationValue,
			String token,String monitorDataType) {

		log.info("{start}---{setConstantTh}----");
		...
		try {
			...
			if (StringUtils.isBlank(presentValue)
					|| StringUtils.isBlank(topLimit)
					|| StringUtils.isBlank(lowerLimit)
					|| StringUtils.isBlank(fluctuationValue)) {
				log.info("设定参数错误");
				throw new WebApiException(WebApiExceptionType.PARAMETERS_ERROR);
			}
			if (StringUtils.isBlank(id)) {
				log.info("设定参数错误 id");
				throw new WebApiException(WebApiExceptionType.ID_ERROR);
				...
				Long deviceId = Long.parseLong(id);
				Long presentValueVal = Long.parseLong(presentValue);
				Long topLimitVal = Long.parseLong(topLimit);
				Long lowerLimitVal = Long.parseLong(lowerLimit);
				Long fluctuationValueVal = Long.parseLong(fluctuationValue);
 
				sendConstantThCmdService.setConstantCmd(deviceId,
						presentValueVal, topLimitVal, lowerLimitVal,
						fluctuationValueVal,monitorDataType);
