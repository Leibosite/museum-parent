...
<head>
<title><fmt:message
		key="pageTitle.data_visualization_statistics_power" /></title>
<link rel="stylesheet"
...
<body>
	...
	<div id="js-ajax-repalce-block" style="position: relative">
		<h3 style="position: relative; color: #666666">
			<fmt:message key="pageTitle.data_visualization_statistics_power" />
			<span
		...
		<form:form class="well-small well form-inline search-form-style"
			...
			<form:select path="monitorStationId" data-placeholder="默认全部监测站"
			...
			<form:select path="monitorPointId" data-placeholder="默认全部监测点"
				cssClass="chzn-autoselect-device">
				<form:option value=""></form:option>
				<c:if test="${not empty deviceId}">
					<form:option value="${device.id}" label="${device.name}"></form:option>
			...
			<table id="tech-companies-1"
				...
				<thead>
					<tr>
						...
						<th data-priority="3"><fmt:message key="th.power" /></th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${pageObjects.content}" var="dataRecord">
						<tr id="changeColor">
							...
							<td><c:out value="${dataRecord.deviceStation.name}" /></td>
							<td><c:out value="${dataRecord.device.name}" /></td>
							<td><c:out value="${dataRecord.power}" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
