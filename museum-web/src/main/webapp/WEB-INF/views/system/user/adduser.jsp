	...
	<script>
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				rules: {
					loginName: {
						remote: "${ctx}/system/user/checkLoginName?oldLoginName=" + encodeURIComponent('${User.loginName}')
					},
					accessGroups:"required"
				},
				messages: {
					loginName: {
						remote: "用户登录名已存在"
...
<body>
	<form:form id="inputForm" modelAttribute="User" action="${ctx}/system/user/save/${User.id}" method="post">
		...
		<fieldset class="prepend-top">
			...
			<div>
				<label for="accessGroups" class="field">权限组列表:</label>
				<form:checkboxes path="accessGroups" items="${allGroups}" itemLabel="group_name" itemValue="id"/>
			</div>
		</fieldset>
		<div>
